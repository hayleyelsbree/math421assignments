...
df <- read_excel("C:\\Users\\student\\Downloads\\c2015.xlsx")
df
```
...
```{r}
df <- df |> mutate(AGE = na_if(AGE, 'Unknown'))
```
...
```{r}
df <- df |> mutate(AGE = str_replace_all(AGE, 'Less than 1', '0'))
```
...
df <- df |> mutate(TRAV_SP = str_remove_all(TRAV_SP, ' MPH'))
table(df$TRAV_SP)
```
...
```{r}
# Use the `str_replace` function to replace `Stopped` by '0' (dont forget the quotation mark around 0)
df <- df |> mutate(TRAV_SP = str_replace_all(TRAV_SP, 'Stopped', '0'))
table(df$TRAV_SP)
```
...
```{r}
# Use `na_if` to change all the forms of missing values to `NAs`
df <- df |> mutate(TRAV_SP = na_if(TRAV_SP, 'Not Rep'))
df <- df |> mutate(TRAV_SP = na_if(TRAV_SP, 'Unknown'))
table(df$TRAV_SP)
```
...
```{r}
# Remove rows in df that aren't driver
df <- df |> filter(SEAT_POS == 'Front Seat, Left Side')
table(df$SEAT_POS)
```
