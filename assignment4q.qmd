...
```{r}
df$weekend <- case_when((df$weekday == "Saturday") | (df$weekday == "Sunday") ~ 1,
                        TRUE ~ 0)
df
```
...
```{r}
df %>%
  filter((month == "October" | month == "November") & year(date) == 2020 & weekend == 1) %>%
  group_by(month) %>%
  summarise(median(positiveIncrease))
```
...
df %>%
  drop_na(death) %>%
  filter((month == "September" | month == "October" | month == "November") & year(date) == 2020) %>%
  group_by(month2) %>%
...
df %>%
  ...
  filter((month == "June" | month == "July" | month == "August") & year(date) == 2020) %>%
  group_by(weekend) %>%
...
```{r}
df2 <- read_csv("https://bryantstats.github.io/math421/data/us-states.csv")
```

```{r}
# Question 14: Find the average cases in Rhode Island in 2021
df2 %>%
...
```{r}
# Question 15: Find the median cases by weekdays in Rhode Island in 2021
...
```{r}
# mutate
df3 |> mutate(elderly = ifelse(age >= 65, "Elderly", "Not Elderly"))
```

```{r}
# summarise
df3 |> summarise(min(age), mean(age), median(age), max(age))
```
...
df3 |> 
  group_by(sex) |> 
  summarise(mean(age), median(age), max(age))
```
...
df3 |> 
  filter(race == "White") |> 
  group_by(income) |> 
  summarise(min(hours.per.week), mean(hours.per.week), median(hours.per.week), max(hours.per.week))
```
